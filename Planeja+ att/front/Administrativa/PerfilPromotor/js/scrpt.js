...
function Carregar() {
    ...
    usuarios.forEach(u => {
        if (u.id_usuario == v_user.id_promotor) {
            ...
            if (u.nomeFantasia == null) {
                document.querySelector('.nome-promotor').innerHTML = u.nome
            } else {
            ...
            tipoEventos.forEach(te => {
                ...
                if (te.id_usuario == u.id_usuario) {
                    ...
                    novoCardTipoEventos.querySelector('.foto_card').src = "../../../back/src/" + u.caminhoImagemEvento
            ...
            locacoes.forEach(l => {
                ...
                if (l.idUsuario == u.id_usuario) {
                    ...
                    if (l.idUsuario == u.id_usuario) {
                        ...
                        novoCardLocacoes.querySelector('.foto_card_locacoes').src = "../../../back/src/" + u.caminhoImagemLocacao
...
const btnAdicionarLocaocao = document.getElementById('btn_adicionar_locacao');
const inputFileLocaocao = document.getElementById('img-locacao');

btnAdicionarLocaocao.addEventListener('click', () => {
    inputFileLocaocao.click();
});

inputFileLocaocao.addEventListener('change', (event) => {
    const file = event.target.files[0];

    const formData = new FormData();
    formData.append('img', file);

    const id_usuario = user.id_user;

    const url = `http://localhost:3000/enviarLoc/${id_usuario}`;
...
inputFile.addEventListener('change', (event) => {
    ...
    const url = `http://localhost:3000/enviarEvento/${id_usuario}`;
